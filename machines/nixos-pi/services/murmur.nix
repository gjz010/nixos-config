    ...
    (pkgs.botamusique.override {
      python3Packages = (pkgs.python3.override {
        packageOverrides = pyself: pysuper: {
          pymumble = (pysuper.pymumble.overrideAttrs (old: {
            pname = "pymumble";
            version = "1.7.0";
          ...
          yt-dlp = (pysuper.yt-dlp.overrideAttrs (old: {
            patches = [ ./11667-yt-dlp-bilibili-fix.patch ];
          }));
        };
      }).pkgs;
    }).overrideAttrs (f: p: rec {
      src = pkgs.fetchFromGitHub {
        owner = "azlux";
        repo = "botamusique";
      ...
      buildPhase = let buildPython = pkgs.python3Packages.python.withPackages (ps: [ ps.jinja2 ]); in ''
        runHook preBuild
        ...
        runHook postBuild
      '';
      patches = (builtins.filter (p: !(lib.strings.hasInfix "catch-invalid-versions" "${p}")) p.patches) ++ [ ./botamusique.patch ];
    });
...
in
{
  ...
  systemd.services.botamusique = {
    serviceConfig = {
      ...
      ExecStart = lib.mkForce "${config.services.botamusique.package}/bin/botamusique --config ${(pkgs.formats.ini {}).generate "botamusique.ini" config.services.botamusique.settings } -C \${CREDENTIALS_DIRECTORY}/botamusique.pem";
    };
  };
  networking.nat = {
